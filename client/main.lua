...
RegisterCommand("opencase", function()
    if openMenuSpamProtect < GetGameTimer() then 
        openMenuSpamProtect = GetGameTimer() + 1500

        local user_id = vRP.getUserId({source})
        if user_id then
            vRP.getUData({user_id, "ak4y-caseOpening"}, function(udata)
                local result = json.decode(udata)
                local firstname = result.charName
                local lastname = ""
                apiKey = result.apiKey
                local steamID = result.steamid and "https://api.steampowered.com/ISteamUser/GetPlayerSummaries/v0002/?key=" .. apiKey .. "&steamids=" .. result.steamid or 'null'
...
RegisterNUICallback("collectItem", function(data, cb)
    if collectItemSpamProtect < GetGameTimer() then 
        ...
        if lastSelected and lastSelected.itemName == data.collectedItem.itemName then 
            TriggerServerEvent("ak4y-caseOpening:collectItem", function(result)
                SendNUIMessage({
                    type = 'setLastItems', 
                    lastItems = result.lastItems,
                })	
...
RegisterNUICallback("sellItem", function(data, cb)
    if sellItemSpamProtect < GetGameTimer() then 
        ...
        if lastSelected and lastSelected.sellCredit == data.sellItem.sellCredit then 
            TriggerServerEvent("ak4y-caseOpening:sellItem", function(result)
                cb(result)
    ...
    if openCaseSpamProtect < GetGameTimer() then 
        ...
        if data then 
            ...
            if lastValidatedCase then 
                ...
                TriggerServerEvent("ak4y-caseOpening:selectedCaseOpen", function(result)
                    if result then 
                        cb(lastSelected)
                    else
                        cb(false)
...
RegisterNUICallback('sendInput', function(data, cb)
    if sendInputSpamProtect <= GetGameTimer() then
        ...
        TriggerServerEvent("ak4y-caseOpening:sendInput", function(result)
            if result then     
                cb(tonumber(result))
            else
                cb(false)
